<!DOCTYPE html>
<html>
<head>
	<title>Livros Cadastrados</title>
	<link rel="stylesheet" href="views/templates/css/style.css">
	<link rel="stylesheet" href="views/templates/css/style2.css">
    <link rel="icon" href="views/templates/img/favicon-16x16.png">
</head>
<header>
<div class="header-cabecalho">
        <div class="botao-voltar">
            <a class="voltar" href="home"><img style="height: 30px; width: 30px" src="views/templates/img/fav-icon_bibli.png" alt="logo"></a>
        </div>
        <div class="texto-cabecalho">
            <h1 class="h1-cadastro">Biblioteca Comunitária Maria Dolores</h1>
        </div>
    </div>
</header>
<body>